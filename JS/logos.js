 ...
 * Icons page (JS/motvin-icons.js).
 *
 * Original dataset uses Lucide-style mock paths. Logo-specific data loading
...
const state = {
  ...
  copyFmt: "svg",
  gridOn: false,
  globalStroke: parseFloat(localStorage.getItem("ml.globalStroke")) || 1.5,
  globalSize: parseInt(localStorage.getItem("ml.globalSize"), 10) || 55,
  globalColor: localStorage.getItem("ml.globalColor") || "currentColor",
  collections: JSON.parse(localStorage.getItem("ml.collections") || "[]"),
  folders: JSON.parse(localStorage.getItem("ml.folders") || "null"),
  activeFolderId: null,
};

if (!state.folders) {
  state.folders = [];
  const oldFavs = JSON.parse(localStorage.getItem("ml.favorites") || "[]");
  if (oldFavs.length > 0) {
    state.folders.push({
      id: "default",
      name: "Favorites",
...
function saveFiltersLS() {
  ...
  localStorage.setItem("ml.sort", state.sort);
...
function applyGlobalColor(hex) {
  ...
  localStorage.setItem("ml.globalColor", state.globalColor);
...
function wire() {
  ...
  if (globalSizeSlider) {
    ...
    globalSizeSlider.addEventListener("input", (e) => {
      ...
      localStorage.setItem("ml.globalSize", val);
  ...
  if (globalStrokeSlider) {
    ...
    globalStrokeSlider.addEventListener("input", (e) => {
      ...
      localStorage.setItem("ml.globalStroke", val);
...
function buildTopCategoryDropdown() {
  ...
  // Update label
  if (state.categoryFilter.size === 0) {
    if (catDropdownVal) catDropdownVal.textContent = "All Logos";
  } else {
