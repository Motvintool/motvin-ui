...
const SOURCE_STYLE_BIAS = {
  ...
  radix: ["outline", "solid"],
  antd: ["outline", "solid", "duotone"],
  fluent: ["outline", "solid"],
  carbon: ["outline", "solid"],
  iconoir: ["outline", "solid"],
  ionicons: ["outline", "solid"],
  fontawesome: ["outline", "solid", "duotone", "brands"],
...
for (const [categoryName, keywords] of Object.entries(CATEGORY_MAP)) {
  CATEGORY_REGEX_MAP[categoryName] = new RegExp(
    "\\b(" + keywords.join("|") + ")\\b",
  );
}
...
const state = {
  ...
  copyFmt: "svg",
  gridOn: false,
  globalStroke: parseFloat(localStorage.getItem("mi.globalStroke")) || 1.5,
  globalSize: parseInt(localStorage.getItem("mi.globalSize"), 10) || 36,
  globalColor: localStorage.getItem("mi.globalColor") || "currentColor",
...
function saveFiltersLS() {
  ...
  localStorage.setItem("mi.sort", state.sort);
  ...
  if (!keepNativeColors && !hasComplexDefs) {
    ...
    cleanPaths = cleanPaths.replace(
      ...
      function (match, tag, attrs) {
        ...
        const pureAttrs = attrs.replace(/\/\s*$/, "");
...
function renderFilters() {
  ...
  if (sourceAllTitle && sourceAllAvatars && badgeLg) {
    ...
    if (state.sourceFilter.size === 0) {
      ...
      sourceAllContainer.title = "Showing all sources";
      sourceAllAvatars.innerHTML = `
        <div class="mi-rp-avatar" style="z-index: 3"><img src="ASSET/Icons/Lucide.svg" alt=""/></div>
        <div class="mi-rp-avatar" style="z-index: 2; margin-left: -6px"><img src="ASSET/Icons/Heroicons.svg" alt=""/></div>
      ...
      const getSourceIcon = (label) => {
        ...
        if (low.includes("hero")) icon = "Heroicons.svg";
        else if (low.includes("lucide")) icon = "Lucide.svg";
        else if (low.includes("simple")) icon = "icons-brand.svg";
        else if (low.includes("phosphor")) icon = "Phosphor.svg";
        else if (low.includes("tabler")) icon = "Tabler Icons.svg";
        return icon;
      };
...
function renderBulkActions() {
  ...
  window.MultiActionsStrip?.render({
    ...
    onCopy: async (format) => {
      ...
      // the first icon and drops the rest - merge them into one document.
      const payload =
        format === "svg"
      ...
      copyText(payload).then((ok) =>
        toast(
          ok
            ? `Copied ${count} ${count === 1 ? "icon" : "icons"}`
            : "Copy failed",
        ),
      );
    },
    onDownload: async (format) => {
      ...
      // One <a download> click per icon only ever produced the first file -
...
function syncEditorControls() {
  ...
  $("#grp-fill-color").style.display = e.fillMode === "solid" ? "flex" : "none";
  $$("[data-cap]").forEach((b) =>
...
function updateShapeControlsVisibility() {
  ...
  $("#grp-icon-inset").style.display = hasShape ? "" : "none";
  $("#grp-shape-color").style.display = hasShape ? "" : "none";
  $("#grp-shape-radius").style.display = isRounded ? "" : "none";
}
...
async function renderCanvas() {
  ...
  const svg = renderSvg(state.editorIcon.svg, editorRenderOpts());
...
// --------------------------------------------------------------------
/**
 * Export markup for one icon. Icons the API served without SVG content render
 * as an `<img>` to the CDN file, which cannot be embedded in a combined SVG or
 * written to a .svg - so fetch the file first, the way the editor does.
 */
async function exportSvgFor(icon) {
  if (!icon.svg && icon.source && icon.name) {
    try {
      const url = window.iconsAPI?.getIconSVGUrl(icon.source, icon.name);
      if (url) {
...
function requireLoginToDownload() {
  ...
  return true; // Fallback if auth service is not loaded
}
...
function applyGlobalColor(hex) {
  ...
  localStorage.setItem("mi.globalColor", state.globalColor);
...
function wire() {
  ...
  if (globalSizeSlider) {
    ...
    globalSizeSlider.addEventListener("input", (e) => {
      ...
      localStorage.setItem("mi.globalSize", val);
  ...
  if (globalStrokeSlider) {
    ...
    globalStrokeSlider.addEventListener("input", (e) => {
      ...
      localStorage.setItem("mi.globalStroke", val);
  ...
  if (searchInput) {
    ...
    const applyPlaceholder = () => {
      searchInput.placeholder = compact.matches
        ? "Search icons..."
        : `Search ${getTotalIconCount().toLocaleString()}+ icons...`;
    };
    applyPlaceholder();
...
function buildCategoryList() {
  ...
  container.querySelectorAll(".mi-rp-cat-item").forEach((item) => {
    item.addEventListener("click", () => {
      ...
      if (typeof buildTopCategoryDropdown === "function")
...
function buildTopCategoryDropdown() {
  const container = document.getElementById("cat-menu-list");
  ...
  if (state.categoryFilter.size > 0) {
    valDisplay.textContent = Array.from(state.categoryFilter)[0];
  } else {
    valDisplay.textContent = "All Icons";
  }

  container.querySelectorAll(".mi-category-menu-item").forEach((item) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initUI = () => {
    ...
    // clearing the skeletons before STATS_LOADED resolves paints a count of 0
    // and drops the loading state the markup ships with.
    const badgeLg = document.querySelector(".mi-rp-badge-lg");
    if (badgeLg) {
      badgeLg.textContent = getTotalIconCount().toLocaleString();
  ...
  if (detachBtn && rightPanel && rpHeader) {
    ...
    const PANEL_LS_KEY = "mi_panel_layout";
...
document
  ...
  ?.addEventListener("click", () => {
    ...
    const input = document.getElementById("local-coll-name");
    if (input) {
      input.value = "";
