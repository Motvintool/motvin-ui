...
(function() {
  ...
  function loadStats() {
    ...
    window.iconsAPI.getStats()
      .then(stats => {
        window.ICON_STATS = stats;
        const loadTime = (performance.now() - startTime).toFixed(0);

        console.log(`[Stats Bridge] ✓ Loaded in ${loadTime}ms`);
        console.log(`[Stats Bridge] Total icons: ${stats.total.toLocaleString()}`);
  ...
  if (window.iconsAPI) {
    ...
    console.error('[Stats Bridge] iconsAPI not available - load api-client.js first');
  }
