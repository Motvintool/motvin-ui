...
(function () {
  ...
  window.getIllustrationFilterCounts = function (type) {
    ...
    switch (type) {
      case "source":
        return (window.ILLUSTRATION_STATS.collections || []).reduce(
          (acc, c) => {
            acc[c.id] = c.total;
            return acc;
          },
      ...
      case "style":
        return window.ILLUSTRATION_STATS.byStyle || {};
      case "license":
        return window.ILLUSTRATION_STATS.byLicense || {};
      case "category":
        return window.ILLUSTRATION_STATS.byCategory || {};
      default:
        return {};
    }
  };
  ...
  function loadInitialIllustrations() {
    ...
    if (typeof window.recreateIllustrations === "function")
      window.recreateIllustrations();
  }
  ...
  function loadStats() {
    ...
    window.illustrationsAPI
      .getStats()
      .then((stats) => {
        stats.total =
          stats.total ||
          (stats.collections || []).reduce((s, c) => s + (c.total || 0), 0);
        window.ILLUSTRATION_STATS = stats;

        const loadTime = (performance.now() - startTime).toFixed(0);
        console.log(
          `[Stats Bridge] ✓ Loaded in ${loadTime}ms — ${stats.total.toLocaleString()} illustrations`,
        );
      ...
      .catch((err) => {
        console.error(
          "[Stats Bridge] ✗ Failed to load illustration stats:",
          err,
        );
  ...
  if (window.illustrationsAPI) {
    ...
    console.error(
      "[Stats Bridge] illustrationsAPI not available — load api-client-illustrations.js first",
    );
  }
})();
