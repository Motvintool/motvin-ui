...
# We want to replace everything inside constructor's this.shadowRoot.innerHTML assignment
import re
new_html = r"""`
            <style>
                :host {
...
# Find the start and end of this.shadowRoot.innerHTML
start_idx = content.find("this.shadowRoot.innerHTML = `")
if start_idx != -1:
    # Find the matching closing backtick of the innerHTML string.
    # We can just look for the first `;` after the `
    end_idx = content.find("`;", start_idx)
    if end_idx != -1:
        new_content = content[:start_idx] + "this.shadowRoot.innerHTML = " + new_html + content[end_idx+1:]
